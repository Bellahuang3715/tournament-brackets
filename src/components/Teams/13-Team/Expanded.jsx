import React from 'react';
import styles from './stylesheet.module.css';

export default function Expanded() {
  return (
    <>
      <table border={0} cellPadding={0} cellSpacing={0} width={1903} style={{borderCollapse: 'collapse', tableLayout: 'fixed', width: '1426pt'}}>
        <colgroup><col width={163} style={{msoWidthSource: 'userset', msoWidthAlt: 5205, width: '122pt'}} />
          <col width={35} span={2} style={{msoWidthSource: 'userset', msoWidthAlt: 1109, width: '26pt'}} />
          <col width={163} style={{msoWidthSource: 'userset', msoWidthAlt: 5205, width: '122pt'}} />
          <col width={35} span={2} style={{msoWidthSource: 'userset', msoWidthAlt: 1109, width: '26pt'}} />
          <col width={163} style={{msoWidthSource: 'userset', msoWidthAlt: 5205, width: '122pt'}} />
          <col width={35} span={2} style={{msoWidthSource: 'userset', msoWidthAlt: 1109, width: '26pt'}} />
          <col width={163} style={{msoWidthSource: 'userset', msoWidthAlt: 5205, width: '122pt'}} />
          <col width={35} span={2} style={{msoWidthSource: 'userset', msoWidthAlt: 1109, width: '26pt'}} />
          <col width={163} style={{msoWidthSource: 'userset', msoWidthAlt: 5205, width: '122pt'}} />
          <col width={101} span={8} style={{width: '76pt'}} />
        </colgroup><tbody><tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl66} width={163} style={{height: '15.75pt', width: '122pt'}} />
            <td className={styles.xl65} width={35} style={{width: '26pt'}} />
            <td className={styles.xl66} width={35} style={{width: '26pt'}} />
            <td className={styles.xl66} width={163} style={{width: '122pt'}} />
            <td className={styles.xl65} width={35} style={{width: '26pt'}} />
            <td className={styles.xl67} colSpan={5} width={431} style={{msoIgnore: 'colspan', width: '322pt'}}>GROUP
              x-x | COURT x</td>
            <td className={styles.xl65} width={35} style={{width: '26pt'}} />
            <td className={styles.xl66} width={35} style={{width: '26pt'}} />
            <td className={styles.xl66} width={163} style={{width: '122pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
            <td className={styles.xl66} width={101} style={{width: '76pt'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl70} style={{height: '15.75pt'}}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td rowSpan={2} className={styles.xl70}>1</td>
            <td className={styles.xl71}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl72} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl73}>&nbsp;</td>
            <td className={styles.xl74}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl65} style={{height: '15.75pt'}} />
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td rowSpan={2} className={styles.xl70}>6</td>
            <td className={styles.xl76}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl77} style={{borderTop: 'none'}}>&nbsp;</td>
            <td className={styles.xl74}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl70} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td rowSpan={2} className={styles.xl70}>2</td>
            <td className={styles.xl71}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl72} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl73}>&nbsp;</td>
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl69} />
            <td />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl65} style={{height: '15.75pt'}} />
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td rowSpan={2} className={styles.xl65}>10</td>
            <td className={styles.xl78}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl79} style={{borderTop: 'none'}}>&nbsp;</td>
            <td className={styles.xl80}>&nbsp;</td>
            <td className={styles.xl81}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl81}>&nbsp;</td>
            <td />
            <td className={styles.xl65} />
            <td className={styles.xl81}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl81} style={{borderLeft: 'none'}}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl81}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl81}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl70} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl81}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td rowSpan={2} className={styles.xl70}>3</td>
            <td className={styles.xl71}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl81}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl72} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl73}>&nbsp;</td>
            <td className={styles.xl74}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl81}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td rowSpan={2} className={styles.xl70}>7</td>
            <td className={styles.xl76}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl65} style={{height: '15.75pt'}} />
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td />
            <td className={styles.xl65} />
            <td />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td rowSpan={2} className={styles.xl70}>12</td>
            <td className={styles.xl78} style={{borderLeft: 'none'}}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl65} />
            <td className={styles.xl66} />
            <td className={styles.xl66} />
            <td className={styles.xl79} style={{borderTop: 'none', borderLeft: 'none'}}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl70} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl72}>&nbsp;</td>
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td rowSpan={2} className={styles.xl70}>4</td>
            <td className={styles.xl71}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl81} style={{borderLeft: 'none'}}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl72} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl73}>&nbsp;</td>
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl72}>&nbsp;</td>
            <td className={styles.xl65} />
            <td />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl81} style={{borderLeft: 'none'}}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl69} />
            <td />
            <td rowSpan={2} className={styles.xl70}>8</td>
            <td className={styles.xl71}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
            <td />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl81} style={{borderLeft: 'none'}}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl65} style={{height: '15.75pt'}} />
            <td className={styles.xl69} />
            <td className={styles.xl72}>&nbsp;</td>
            <td className={styles.xl73}>&nbsp;</td>
            <td className={styles.xl70}>&nbsp;</td>
            <td />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl81} style={{borderLeft: 'none'}}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl73}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl70}>&nbsp;</td>
            <td />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl69} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl73}>&nbsp;</td>
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl70}>&nbsp;</td>
            <td />
            <td className={styles.xl66} />
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl69} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td />
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl72} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td rowSpan={2} className={styles.xl70}>11</td>
            <td />
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black'}}>REN B</td>
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl82}>&nbsp;</td>
            <td className={styles.xl80}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl70} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl72}>&nbsp;</td>
            <td className={styles.xl65} />
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl70}>&nbsp;</td>
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td rowSpan={2} className={styles.xl70}>5</td>
            <td className={styles.xl71}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl68}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl69} />
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl72} style={{height: '15.75pt'}}>&nbsp;</td>
            <td className={styles.xl73}>&nbsp;</td>
            <td className={styles.xl70}>&nbsp;</td>
            <td className={styles.xl69} />
            <td className={styles.xl72}>&nbsp;</td>
            <td className={styles.xl70}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td rowSpan={2} height={40} className={styles.xl112} style={{borderBottom: '.5pt solid black', height: '31.5pt', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
            <td className={styles.xl69} />
            <td />
            <td rowSpan={2} className={styles.xl70}>9</td>
            <td className={styles.xl71}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl65} style={{height: '15.75pt'}} />
            <td className={styles.xl69} />
            <td className={styles.xl72}>&nbsp;</td>
            <td className={styles.xl73}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl73}>&nbsp;</td>
            <td rowSpan={2} className={styles.xl112} style={{borderBottom: '.5pt solid black', borderTop: 'none'}}>REN B</td>
            <td className={styles.xl75}>&nbsp;</td>
          </tr>
          <tr height={20} style={{msoHeightSource: 'userset', height: '15.75pt'}}>
            <td height={20} className={styles.xl69} style={{height: '15.75pt'}} />
            <td className={styles.xl65} />
            <td className={styles.xl73}>&nbsp;</td>
          </tr>
          <tr height={17} style={{height: '13.0pt'}}>
            <td height={17} style={{height: '13.0pt'}} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
            <td className={styles.xl65} />
            <td colSpan={2} style={{msoIgnore: 'colspan'}} />
          </tr>
        </tbody></table>
    </>
  );
}
